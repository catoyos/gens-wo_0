...
public class MyAIEngineIndividual {
	...
	public static boolean[] agreeToPair(Individual indA, Individual indB, float moment) {
		...
		int familydgAB = getFamilyDegree(indA, indB);
		t1 = System.currentTimeMillis();
		int familydgBA = getFamilyDegree(indB, indA);
		t2 = System.currentTimeMillis();
		if(familydgAB != familydgBA) System.out.println("AAA,"+(t1-t0)+","+(t2-t1)+",getFamilyDegreeEEEERRRRROORRRR -> #"+indA.getIndividualID() + "," + indB.getIndividualID() + "," + familydgAB + "," + familydgBA);
		else System.out.println("AAA,"+(t1-t0)+","+(t2-t1)+",getFamilyDegree BIENNN");
...
//		return individual.getHorniness() * auxnn / Math.max(32f - sexOrientation, 32f + sexOrientation);
	...
	public static int getFamilyDegree(Individual indA, Individual indB){
		...
		if (indA.getNChildren() < indB.getNChildren()) {
			...
			relatives1dg.add(indA.getFatherID());
			relatives1dg.add(indA.getMotherID());
		} else {
			tgt = indA;
			centerid = indB.getIndividualID();
			relatives1dg = new HashSet<String>(indB.getChildrenID());
			relatives1dg.add(indB.getFatherID());
			relatives1dg.add(indB.getMotherID());
		}
		...
		for (String rel : relatives1dg) {
			...
			if (!centerid.equals(auxid) && !relatives1dg.contains(auxid)) {
			...
			for (String child : auxlist) {
				if (!centerid.equals(child) && !relatives1dg.contains(child)) {
					relatives2dg.add(auxid);
				}
			}
		}
